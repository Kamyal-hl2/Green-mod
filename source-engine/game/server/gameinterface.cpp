		...
		CGlobalVars *pGlobals)
{
	...
	// Green Engine v2: bootstrap the Lua VM. Must happen after all engine
	// interfaces above are connected (filesystem in particular, since
	// LuaVM_Bootstrap() loads hook.lua through it) but before PostInit(),
	// which is where gamemode/autorun scripts get loaded.
	if ( !LuaVM_Bootstrap() )
	{
		Warning( "CServerGameDLL::DLLInit: Lua VM bootstrap failed\n" );
	}
...
void CServerGameDLL::PostInit()
{
	...
	// Green Engine v2: gamemode entry point + autorun scripts. Currently a
	// stub (see lua_vm.cpp) until ents.* lands — the sandbox gamemode's
	// init.lua calls ents.* immediately on load.
	LuaVM_LoadGameScripts();
}

void CServerGameDLL::DLLShutdown( void )
{
	// Green Engine v2: close the Lua VM before the rest of DLLShutdown tears
	// down engine systems it might reference (filesystem, etc).
	LuaVM_Shutdown();
...
void CServerGameDLL::GameFrame( bool simulating )
{
	...
	// Green Engine v2: bridge to hook.Run("Think") on the Lua side. This is
	// the primitive the whole GMod Lua layer hangs off of — see the risk
	// note in the project plan about throttling heavy hooks; unthrottled
	// for the MVP.
	LuaVM_RunHooks( "Think" );
