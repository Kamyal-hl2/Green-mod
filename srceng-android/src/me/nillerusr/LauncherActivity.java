...
public class LauncherActivity extends Activity {
	...
	// Green Engine v2: MANAGE_EXTERNAL_STORAGE is a "special app access"
	// permission — unlike everything in applyPermissions() below, it can't
	// be granted through the normal requestPermissions() runtime dialog.
	// It only shows up via a dedicated Settings screen. Call this instead
	// of/alongside applyPermissions() on API 30+; below that, the existing
	// WRITE_EXTERNAL_STORAGE flow (still requested via applyPermissions()
	// for maxSdkVersion 29 in the manifest) is what actually grants access.
	public void applyManageStoragePermission() {
		if ( Build.VERSION.SDK_INT >= 30 && !Environment.isExternalStorageManager() ) {
			try {
				...
				// Some OEM builds don't ship the per-app variant of this
				// screen — fall back to the general "All files access" list.
...
//		check_updates.setChecked(mPref.getBoolean("check_updates", true));
		...
		// Green Engine v2: separate special-permission flow, see the method
		// comment above for why this can't just be folded into the call
		// above.
		applyManageStoragePermission();
/*
