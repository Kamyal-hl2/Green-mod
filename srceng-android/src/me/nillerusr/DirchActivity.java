...
public class DirchActivity extends Activity implements OnTouchListener{
	...
	public void ListDirectory( String path )
	{
		...
		if (directories != null && directories.length > 1) {
			// ОПТИМИЗАЦИЯ: Используем String.CASE_INSENSITIVE_ORDER вместо
			// toUpperCase() в каждом сравнении. toUpperCase() создаёт новый String
			// на каждый вызов compare() — O(n log n) аллокаций при сортировке.
			// String.CASE_INSENSITIVE_ORDER делает то же самое без аллокаций.
	...
	public List<String> getExtStoragePaths() {
		...
		if (sdk >= 30) {
			// On API 30+ /storage/ is not accessible without MANAGE_EXTERNAL_STORAGE
			return list;
		}
